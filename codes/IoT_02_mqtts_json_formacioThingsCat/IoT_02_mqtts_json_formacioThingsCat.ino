...
#include <ESPmDNS.h>
WiFiClientSecure espClient; // Cal afegir: espClient.setCACert(ca_cert);
// WiFiClient espClient;  // Cal comentar: espClient.setCACert(ca_cert);
...
void receivedCallback(char* topic, byte* payload, unsigned int length) {
  ...
  if (szTopic == String("/" + String(sMac) + TOPIC_JSON_BME_REQ).c_str()) {
    String szJson = JSON.stringify(jsonBME);
    Serial.print("JSON: "); Serial.println(szJson);
    client.publish( String("/" + String(sMac) + TOPIC_JSON_BME).c_str(), szJson.c_str());
  }  
    ...
    if (now > lastNow + 1000) // Every second
    {
      ...
      if (fP > 800 && fP < 1300) {
        if (fT > -100.0) {
          ...
          json_IoT["Tx100"] = nTx100; json_IoT["RHx100"] = nRHx100; json_IoT["Px100"] = nPx100; json_IoT["Ax100"] = nAx100;
          nTimes = 0;
          /*
