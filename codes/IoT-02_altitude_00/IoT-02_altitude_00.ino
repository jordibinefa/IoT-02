    ...
    /*
      String sJson()
        ...
        if (jsonBME.hasOwnProperty("Ax100")) {
          ...
          String szAlt = String(fAlt);
          Serial.print("A: "); Serial.print(szAlt); Serial.println(" m");
          client.publish( String("/" + String(sMac) + TOPIC_JSON).c_str(), szAlt.c_str());
        }
    */
  }
...
String sJson() {
  ...
  if (fP > 800 && fP < 1300) {
    if (fT > -100.0) {
      ...
      json_IoT02["T"] = fT;json_IoT02["RH"] = fRH;json_IoT02["P"] = fP;json_IoT02["A"] = fA;json_IoT02["L"] = analogRead(LDR);

      //String jsonString = JSON.stringify(jsonBME);
      //Serial.print("JSON.stringify(jsonBME) = ");
      String jsonString = JSON.stringify(json_IoT02);
      //Serial.print("JSON.stringify(json_IoT02) = ");
