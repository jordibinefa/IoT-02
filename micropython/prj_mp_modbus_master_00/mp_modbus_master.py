...
class modbus_rtu_master(modbus_master):
    def __init__(self, uart_no: int=1, baudrate: int=9600, bits: int=8, stop: int=1, 
        ...
        # super(modbus_rtu_master, self).__init__(ms_type="rtu", *args, **kwargs)        # <-- This doesn't work
        super(modbus_rtu_master, self).__init__(*args, **kwargs)
        self.ms_type="rtu"                                                               # <-- Setting ms_type
    ...
    def _send(self, frame: bytearray) -> bytearray:
        ...
        if mp:
            ...
            while (self.uart.any() == 0) & (i < 1):
                time.sleep(0.05)
                i += 0.05
            # print("self.uart.any(): %d | i: %d "% (self.uart.any(),i))
            if self.uart.any() > 0:
    ...
    async def _send_async(self, frame: bytearray) -> bytearray:
        ...
        if mp:
            ...
            while (self.uart.any() == 0) & (i < 1):
                ...
                i += 0.05
            if self.uart.any() > 0:
                resp = self.uart.readline() # Maybe it should be changed by resp = self.uart.read()
            else:
                raise ValueError("Timeout 3")
        else:
            self.uart.write(frame)
            i = 0
            while (self.uart.inWaiting() == 0) & (i < 1):
