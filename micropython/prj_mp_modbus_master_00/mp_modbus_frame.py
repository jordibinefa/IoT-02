...
class modbus_rtu_frame(modbus_frame):
    ...
    def _create_frame(self):
        ...
        # print("self.frame without CRC: ", self.frame)
    ...
    def parse_frame(cls, frame, fr_type=None):
        ...
        device_addr = frame[0]
        func_code = frame[1]
        # print("Device Address: 0x%X" % device_addr)
        # print("Function Code : 0x%X" % func_code)
        ...
        raise ValueError("l_202) Could not parse Frame " + " ".join(["{:02x}".format(x) for x in frame]))
...
class modbus_tcp_frame(modbus_frame):
    ...
    def parse_frame(cls, frame):
        ...
        print(" ".join(["{:02x}".format(x) for x in frame]))
        raise ValueError("l_378) Could not parse Frame " + " ".join(["{:02x}".format(x) for x in frame]))
